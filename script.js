// ====== Reproductor de URLs ======
// Almacenamiento local: localStorage. Compartir: playlist codificada en el link (?playlist=...)
...
function renderPlayer() {
  ...
  if (item.type === "video") {
    playerStage.innerHTML = `<video id="mainVideo" src="${escapeHtml(previewUrl)}" controls autoplay playsinline></video>`;
    ...
    playerStage.innerHTML = `<img src="${escapeHtml(previewUrl)}" alt="${escapeHtml(item.name)}" />`;
  ...
  // Resalta el item activo en la lista
...
playPauseBtn.addEventListener("click", () => {
  ...
  if (video) {
    ...
    // imagen u otro: alterna estado y temporizador de avance
    setPlayingUi(!isPlaying);
...
function render() {
  ...
  // Mostrar más recientes primero en la lista de gestión (aunque el reproductor use orden de subida)
...
// ================= COMPARTIR (link con playlist codificada) =================
