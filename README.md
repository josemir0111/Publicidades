...
## Funcionalidades
- 🎬 **Reproductor principal** siempre visible arriba: reproduce en orden todas las URLs que agregues (videos con controles nativos, imágenes con avance automático cada 4s, otros enlaces con acceso directo).
...
- ⏮️▶️⏭️ Controles de anterior / reproducir-pausar / siguiente, con avance automático opcional al terminar cada video o imagen.
- 🔗 **Obtener link para compartir**: genera una URL que lleva codificada toda tu playlist. Quien la abra ve el mismo reproductor con tus URLs cargadas, sin tener que agregar nada — no requiere base de datos ni backend.
...
Al hacer clic en "Obtener link para compartir", el sistema convierte tu lista de URLs a JSON, la codifica en Base64 y la agrega como parámetro `?playlist=...` a la URL de tu sitio. Al abrir ese link, la página detecta el parámetro, decodifica la playlist y la carga automáticamente en el reproductor.

**Importante:** esto significa que el link solo funciona una vez que el sitio esté desplegado en una URL pública (por ejemplo, en Vercel). Compartir el link de un `index.html` abierto localmente (`file:///...`) no funcionará para otra persona, porque no hay una URL pública detrás. Además, si tu playlist es muy larga, el link puede volverse demasiado largo para algunos navegadores; en ese caso, usa mejor "Exportar" y comparte el archivo `.json` junto con el sitio desplegado.
